...
function doneToDos(text){
    //const currentValue = toDos[this.id];//제대로 된 값이 나오지 않음(undifined)
    var listItem = this.parentNode;
    paintToDo2(listItem);
    deleteToDo(this);
}//완료된 리스트 구현할 부분, 정상값이 옮겨지지 않음
let idNumbers= 1;
...
function deleteToDo2(event){
    ...
    saveDoneList();
} //완성된 리스트 내용 삭제->가장 아래부터 순서대로 삭제되는듯

function paintToDo2(text){ //코드 겹치는 부분 없이 구현하는 방향으로 더 고민해보기
